...
/*!
  ...
  \brief A filter for fixed rotate.

For free rotate, use FreeRotate instead.

Supports options: Angle (rotate angle, this must be integer divisible
by 90 otherwise it is ignored.)

Default behavior: rotates 90 degrees clockwise.
*/

#ifndef __QUILL_FILTERS_ROTATE_H__
#define __QUILL_FILTERS_ROTATE_H__

#include "quillimagefilterimplementation.h"

class RotatePrivate;

class Rotate : public QuillImageFilterImplementation
{
public:
    Rotate();
    ~Rotate();
    ...
    virtual const QString name() const { return QuillImageFilter::Name_Rotate; }
